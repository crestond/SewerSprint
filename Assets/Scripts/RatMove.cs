...
public class RatAI_TagCheck : MonoBehaviour
{
    ...
    [SerializeField] private Transform groundCheck;         // Empty GameObject near rat's feet for edge detection
    ...
    [SerializeField] private float stopChaseRange = 7f;     // Distance to stop chasing
    ...
    private void Patrol()
    {
        ...
        // Cast a ray downward to check for ground with the "Ground" tag
        RaycastHit2D hit = Physics2D.Raycast(groundCheck.position, Vector2.down, groundCheckDistance);

        // If no collider OR the collider doesn't have the "Ground" tag → flip direction
    ...
    {
        // Draw chase radius and ground ray
...
public class RatAI_TagCheck : MonoBehaviour
{
    ...
    private bool IsWallAheadTag()
    {
        ...
        // If your walls also use "Ground", give them a separate tag or switch to a LayerMask here.
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        if (wallCheck)
        {
            ...
            // Use startMovingRight in edit mode so gizmo points right by default
